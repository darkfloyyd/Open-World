...
class OW_Admin
{
	...
	public function render_languages(): void
	{
		...
		if ($notice)
			delete_transient('ow_lang_notice');
	...
	public function render_settings(): void
	{
		...
		if ($scan_result)
			delete_transient('ow_last_scan_result');
		?>
		<div class="wrap ow-wrap">
			<div class="ow-page-header">
				<img src="<?php echo esc_url(OW_PLUGIN_URL . 'assets/images/OpenWorldTransparentCompressed.png') ?>" alt="Open World" class="ow-logo">
			...
			<div class="ow-settings-grid">
				...
				<div class="ow-settings-card" style="grid-column: span 2;">
					...
					<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')) ?>" style="margin-top:12px">
						...
						<textarea name="ow_rewrite_exclusions" style="width:100%; height:120px; font-family:monospace; padding:8px;" placeholder="e.g. /my-account/special-endpoint
https://example.com/api" spellcheck="false"><?php echo esc_textarea(get_option('ow_rewrite_exclusions', '')); ?></textarea>
	...
	public function ajax_deepl_translate(): void
	{
		...
		if (!$lang)
			wp_send_json_error('Missing language.');
	...
	public function handle_save_rewrite_settings(): void
	{
		check_admin_referer('ow_save_rewrite_settings', 'ow_rewrite_settings_nonce');
		if (!current_user_can('manage_options'))
			wp_die('Unauthorized');

		// Aggressive sanitization of textarea to prevent malicious JS or HTML execution
		$raw_exclusions   = isset($_POST['ow_rewrite_exclusions']) ? sanitize_textarea_field(wp_unslash($_POST['ow_rewrite_exclusions'])) : '';
		$clean_exclusions = implode("\n", array_filter(array_map('trim', explode("\n", $raw_exclusions))));
	...
	public function handle_import_po(): void
	{
		...
		if (!OW_Languages::is_valid($lang))
			wp_die('Invalid language');
		if (empty($_FILES['po_file']['tmp_name']))
			wp_die('No file');  // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		if (strlen($content) > 10 * 1024 * 1024)
			wp_die('File too large (max 10MB)');
	...
	private function get_num_plural_forms(string $lang): int
	{
		...
		if (in_array($lang, ['pl', 'ru', 'uk', 'cs', 'sk'], true))
			return 3;
		// Arabic: 6 forms
		if ($lang === 'ar')
			return 6;
